...
### 📎 7. 多模態文件上傳、截圖貼上與多動作批次執行 (Multimodal & Multi-Action)

* **📎 檔案與截圖輸入方式**：
  * **點擊迴紋針按鈕 (📎)**：選擇本機 Markdown、TXT、JSON、CSV、PDF 或圖片（PNG/JPG/WEBP）。
  * **剪貼簿即貼 (Cmd+V / Ctrl+V)**：直接截圖後於輸入框貼上，自動轉為圖片附件。
  * **模型切換**：可選用 `🖼️ Qwen VL Max (視覺多模態)`，直接叫 AI 識別手繪白板或 UI 原型開單。
* **✨ 多動作連鎖批次套用 (Approve All)**：
  * 當 AI 同時產出多項操作（例如同時更新 Charter 與建立 10 張工單），前端顯示 `(0/N 已完成)` 動作陣列。
  * 點擊 **【✨ 一鍵依序執行全部動作 (Approve All)】**，系統會依序安全完成所有原子入庫，無需逐個點擊！
...
1. **多模型切換器 (Model Switcher)**：
   ...
   * **🖼️ Qwen VL Max**：**多模態視覺模型**，支援圖片、架構白板、UI 原型、會議手稿截圖識別。
