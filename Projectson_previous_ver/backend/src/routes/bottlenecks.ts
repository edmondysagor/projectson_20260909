...
router.get('/', async (req: Request, res: Response) => {
  try {
    const result = await query("SELECT * FROM project_item WHERE item_type = 'Bottleneck' ORDER BY id ASC");
...
router.get('/project/:projectId', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query("SELECT * FROM project_item WHERE related_context_id = $1 AND item_type = 'Bottleneck' ORDER BY id ASC", [projectId]);
    res.json(result.rows);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await query("SELECT * FROM project_item WHERE id = $1 AND item_type = 'Bottleneck'", [id]);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      ...
      ) VALUES ($1, $2, $3, $4, 'Bottleneck', $5, $6, $7, $8, $9, $10, $11) RETURNING *`,
      [
        item_display_id,
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const current = await query("SELECT * FROM project_item WHERE id = $1 AND item_type = 'Bottleneck'", [id]);
    ...
    const finalContext = related_context_id || dbB.related_context_id;
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await query("DELETE FROM project_item WHERE id = $1 AND item_type = 'Bottleneck' RETURNING *", [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Bottleneck not found' });
    }
