...
router.get('/', async (req: Request, res: Response) => {
  try {
    const result = await query("SELECT * FROM project_item ORDER BY id ASC");
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await query("SELECT * FROM project_item WHERE id = $1", [id]);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const { item_title, related_context_id, workspace_id, item_type, item_status, item_priority, item_planned_start_date, item_planned_end_date, item_content, item_attribute, item_follow_by, item_assigned_by, parent_item_id, related_item_id_relation } = req.body;
    ...
    if (!finalContextId) {
      ...
      if (!workspaceId) {
        ...
        const wsRes = await query("SELECT workspace_id FROM workspace ORDER BY workspace_id ASC LIMIT 1");
        workspaceId = wsRes.rows.length > 0 ? wsRes.rows[0].workspace_id : 1;
      }
    } else {
      // We have finalContextId, but maybe no workspaceId
      if (!workspaceId) {
        const contextRes = await query("SELECT related_workspace_id FROM project_context WHERE id = $1", [finalContextId]);
    ...
    // 3. Format the new item_display_id (e.g. AAP-001)
    const item_display_id = `${prefix_code}-${String(last_item_number).padStart(3, '0')}`;

    const result = await query(
      `INSERT INTO project_item (item_display_id, item_title, workspace_id, related_context_id, item_type, item_status, item_priority, item_planned_start_date, item_planned_end_date, item_content, item_attribute, item_follow_by, item_assigned_by, parent_item_id, related_item_id_relation)
      ...
      [
        ...
        parent_item_id || null,
        related_item_id_relation ? JSON.stringify(related_item_id_relation) : '[]'
      ]
    );
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const { 
      ...
      related_context_id,
      parent_item_id,
      related_item_id_relation
    } = req.body;

    // Get current record
    const current = await query('SELECT * FROM project_item WHERE id = $1', [id]);
    ...
    const finalContextId = related_context_id || dbTask.related_context_id;
    ...
    // Sync parent_item_id
    const finalParentId = parent_item_id !== undefined ? parent_item_id : dbTask.parent_item_id;

    // Sync related_item_id_relation array
    let finalRelations = related_item_id_relation !== undefined ? related_item_id_relation : (Array.isArray(dbTask.related_item_id_relation) ? [...dbTask.related_item_id_relation] : []);
    ...
    const result = await query(
      `UPDATE project_item
       SET item_title = $1,
           ...
           item_update_log = $12,
           item_comment = $13,
           related_context_id = $14,
           parent_item_id = $15,
           related_item_id_relation = $16,
           item_assigned_by = $17,
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const result = await query("DELETE FROM project_item WHERE id = $1 RETURNING *", [id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Task not found' });
    }
