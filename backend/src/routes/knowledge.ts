...
router.get('/', async (req: Request, res: Response) => {
  try {
    const typeFilter = 'Knowledge' === 'All' ? '' : "WHERE item_type = 'Knowledge'";
...
router.post('/', async (req: Request, res: Response) => {
  try {
    const body = req.body || {};
    const item_title = body.item_title || body.title || body.term || 'New Term';
    ...
    const item_type = body.item_type || body.nature || 'Knowledge';
    const item_status = body.item_status || body.status || 'Active';
